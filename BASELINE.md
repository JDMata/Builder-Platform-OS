...
**Update, same day:** item (2)'s flagged gap was closed — a real browser drove the Login → Idea Submission handoff for the first time, and it found the login step **completely broken for every real user** (an OIDC nonce-validation defect, compounded by a redirect-target bug that then crashed `api-gateway` outright). Both fixed and re-verified with a real browser reaching the real Idea Submission screen. See [10-vs1-exit-gate-report.md](docs/execution/sprint-1/10-vs1-exit-gate-report.md) §16 for the full account. **This means the "Business objective achieved" claim in that report's original Exit Gate checklist was not actually true at closure** — it was true of the code and automated tests, not of the real user journey. The baseline below is not reopened or reversed; this is the record of an already-disclosed uncertainty resolving, and what it found.
