...
**Sprint 1 — Discovery Workspace (Release R0.2), Theme: Intelligent Project Discovery.** Status: **Execution Ready, implementation starting.** Planning, UX, and the Product Design Review ([docs/governance/sprint-1-product-design-review/](docs/governance/sprint-1-product-design-review/README.md), APPROVED WITH MINOR UX IMPROVEMENTS — **unchanged, not reopened**) are complete. The execution plan ([docs/execution/sprint-1/](docs/execution/sprint-1/README.md)) was revised 2026-07-16: VS-1 and VS-2 merged into **one Vertical Slice, VS-1 — Discovery Workspace**, now explicitly including Login, Dashboard, Project Type selection, and an Initial Digital Twin/Project Workspace, while preserving the Discovery-first design (a `Project` is created only once Discovery reaches confidence and the user confirms — never an upfront form). The next update to this section happens when the first engineering task (Task 1.3, per [docs/execution/sprint-1/08-implementation-order.md](docs/execution/sprint-1/08-implementation-order.md)) is actually picked up.
...
**One Epic, one Vertical Slice** (revised 2026-07-16 from the earlier 2-Epic structure — see [docs/execution/sprint-1/01-sprint-1-backlog.md](docs/execution/sprint-1/01-sprint-1-backlog.md)'s "Why one Epic and one Vertical Slice now, not two"): **Epic 1 — Discovery-to-Project Delivery**, containing **Vertical Slice VS-1 — Discovery Workspace**, the sprint's single, complete, end-to-end business capability (login through Initial Project Workspace). The Technical Debt & Platform Closure workstream remains embedded as tasks within it, not its own Epic.

## Current Stories

**Organized as Epic → Vertical Slice → Engineering Task, not Story** — 23 engineering tasks under VS-1 ([docs/execution/sprint-1/02-vertical-slice-catalog.md](docs/execution/sprint-1/02-vertical-slice-catalog.md)). The original 18 SAF-39–56 tickets plus 4 new ones added 2026-07-16 (SAF-57 Dashboard, SAF-58 richer project-identifying fields including an opaque, Platform-Pack-scoped Project Type, SAF-59 Digital Twin write integration, SAF-60 Initial Project Workspace) are preserved for traceability in [docs/backlog/sprint-1-backlog.md](docs/backlog/sprint-1-backlog.md), each mapping to exactly one engineering task. SAF-34/SAF-35 (Digital Twin domain + `GraphStorePort` adapter) are pulled forward from Sprint 7, explicitly scoped down to only the 5 node/relationship types this slice needs — their full originally-planned scope (search indexing, semantic embeddings, Governance node types) remains Sprint 7 work.
...
- **Sprint 1 Execution Package revised: VS-1/VS-2 merged into one Vertical Slice** (2026-07-16) — a proposed form-based "Project Creation" request, which would have bypassed Discovery and pulled forward SAP-specific fields into the Kernel, was reconciled instead of adopted as-is: the Product Design Review's Discovery-first design stands **unchanged, not reopened**; the execution package was updated so VS-1 ("Discovery Workspace," 23 tasks) now includes Login, Dashboard, an opaque Platform-Pack-scoped Project Type field, and a minimal Digital Twin/Project Workspace pulled forward from Sprint 7 — justified under the existing "horizontal work pulled forward when the active slice needs it" principle, with no new ADR and no reopened Architecture Review. First engineering task: Task 1.3 (`context-requirements` domain). **Implementation has not started.**
