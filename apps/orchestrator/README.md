...
**Two Postgres roles, deliberately** (mirrors `persistence-postgres/identity`'s finding): `main.ts` uses an admin/superuser pool once, only to call `ensureSchema()`, then a non-superuser `saf_app` pool for the actual running `PostgresOutboxAdapter` — using the superuser for the latter would silently defeat Row-Level Security the moment this context's tables gain RLS policies of their own.
