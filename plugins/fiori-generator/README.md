...
## Ports
Implements `CapabilityPlugin` from `@sap-app-factory/plugin-sdk` — not a `packages/ports` port; plugins implement the `plugin-sdk` lifecycle contract, never `packages/ports` directly (ADR-0006). Passes `testing-kit`'s `capabilityPluginContractTests` for real — not a fake standing in for a future implementation.
