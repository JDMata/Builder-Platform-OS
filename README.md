...
**Status: Sprint 0 Baseline v1.0 established (2026-07-15)** — tag `sprint0-baseline-v1.0`. The Sprint 0 Exit Gate ([docs/governance/sprint-0-exit-gate/](docs/governance/sprint-0-exit-gate/README.md)) returned **GO WITH MINOR CORRECTIONS**; the architecture is now frozen per [ARCHITECTURE_FREEZE.md](ARCHITECTURE_FREEZE.md) — see [BASELINE.md](BASELINE.md) for the full consolidated baseline (repository, architecture, bounded contexts, capability/event catalogs, technology stack, accepted debt, deferred decisions, current limitations) and [ARCHITECTURE_DECISION_INDEX.md](ARCHITECTURE_DECISION_INDEX.md) for every ADR with its affected packages and impact level. Sprint 1 begins against this baseline; any architectural change from this point requires an ADR, architecture review, and impact analysis first. See [docs/backlog/sprint-0-backlog.md](docs/backlog/sprint-0-backlog.md) for per-story progress and [PLATFORM_MATURITY.md](PLATFORM_MATURITY.md) for where the platform stands sprint by sprint. Every story follows Explain → Design → Implement → Test → Document → ADR-update, per [CONTRIBUTING.md](CONTRIBUTING.md) and [TECHNICAL_DEBT_POLICY.md](TECHNICAL_DEBT_POLICY.md).
...
| [docs/adr/README.md](docs/adr/README.md) | Architecture Decision Records index (22 ADRs — 12 original, 6 added by the self-review, 1 for execution profiles, 1 for the AI Workspace, 1 for the Digital Twin, 1 for the Capability Model, several cross-referenced); see also [ARCHITECTURE_DECISION_INDEX.md](ARCHITECTURE_DECISION_INDEX.md) for the same 22 ADRs with affected packages, related ADRs, and impact level |
