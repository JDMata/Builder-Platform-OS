...
[ADR-0022](0022-capability-model-provider-abstraction.md) changes how an `AgentDefinition` is invoked, not its own shape: it is never named directly by a workflow `Step` again, and instead declares `providesCapabilities: CapabilityId[]`, becoming one of possibly several providers for a capability. The eleven required fields this ADR established are unchanged and now map onto the capability-resolution chain's "Implementation" (`Prompt version`) and "Resources" (`Allowed MCP tools`/`Tool permissions`) — see [18-capability-model.md](../architecture/18-capability-model.md).
