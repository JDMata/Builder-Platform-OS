...
[ADR-0022](0022-capability-model-provider-abstraction.md) reframes plugins as **providers** of capabilities rather than the sole, workflow-hardcoded fulfiller of a step: a `WorkflowDefinition` now references a `Capability`, resolved at run time to a `CapabilityProvider` that may be a plugin, an agent, or (in future) a human or external service. Nothing about plugin isolation, the scoped capability token, or the loader changes — only what a workflow step is allowed to name directly (never a specific plugin, exactly as this ADR already required — see [05-plugin-architecture.md](../architecture/05-plugin-architecture.md)).
