...
| **1.5** | Real Anthropic API call behind `LlmProviderPort`. | Backend Engineer | | ✓ | | ✓ | | ✓ | ✓ | None. | Passes `llmProviderContractTests` against the real call; resilience verified; credentials read through the existing `SecretsVaultPort` interface (never hardcoded inline) — its current dev-only `.env`-default adapter is sufficient (see [BASELINE.md](../../../BASELINE.md)'s accepted technical debt: a real secrets-management adapter is triggered by a second provider type or secret rotation, neither of which this task introduces). Building a new `SecretsVaultPort` adapter is explicitly **not** required this task. | M |
...
| **1.8** | `SubmitBusinessIdea` use case — idea text + Workspace reference only. | Backend Engineer | | ✓ | | | | ✓ | | 1.3, 1.4. | Draft `RequirementDocument` persisted; empty idea text rejected; the acting user's permission to submit within the target `Workspace` is checked via the existing, real `PolicyEnginePort` (OPA) adapter — not inferred from `RequestContext` alone (authentication is not authorization). | S |
...
| **1.14** | `ApproveRequirementDocument` use case — confirmation gating, `Project` creation (using the agent-derived name), `requirements.document.captured.v1` emission. | Backend Engineer | | ✓ | | | | ✓ | | 1.8, 1.13. | Rejects confirmation with any unanswered `Clarification`; creates exactly one `Project`; event verifiably in the outbox; the acting user's permission to confirm/approve is checked via the existing, real `PolicyEnginePort` adapter, same discipline as Task 1.8. | M |
